...
// may run concurrently. Separate prefix from lead-intake so the two demos
...
const SYSTEM_PROMPT = `You extract structured fields from the text of a receipt, invoice, or order confirmation for a small business's bookkeeping.
Read the pasted text and return only the structured fields you're asked for.
Extract values only if they literally appear in the text — never invent a vendor name, date, total, invoice number, or line item.
Amounts should be copied as they appear in the text, including currency symbols, rather than reformatted.
If the text has no discernible line items, return an empty list rather than guessing at one.`;
...
export default async function handler(req, res) {
  ...
  if (typeof text !== "string" || text.trim().length === 0) {
    return res.status(400).json({ error: "VALIDATION_ERROR", message: "Paste some receipt or invoice text first." });
  }
  ...
  try {
    ...
    // guard.
    console.error("parse: rate limiter unavailable, proceeding without it", error);
  }
  ...
  try {
    ...
    if (response.stop_reason === "refusal" || !response.parsed_output) {
      return res.status(422).json({
        error: "PARSE_ERROR",
        message: "Couldn't extract fields from that text — try pasting the receipt or invoice content directly.",
      });
    }
    ...
    if (error instanceof Anthropic.APIError) {
      console.error("parse: Anthropic API error", error.status, error.message);
      return res.status(502).json({ error: "UPSTREAM_ERROR", message: "The extractor is temporarily unavailable." });
    }
